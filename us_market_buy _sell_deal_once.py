...
with open(output_file, 'a') as f:
    print(stock_name, " begin to run buy and sell deal !!!!!!!!!!!!!!!!!!!!! ", file=f)
    ...
    if ret == RET_OK:
        if data.shape[0] > 0:  # 如果持仓列表不为空
            ...
            if stock_name in code:
                ...
                current_number = 0
            #### 如果当前仓位等于基准仓位,如果等于说明卖出成功
            if current_number == basic_number:
                with open(output_file, 'a') as f:
                    print(stock_name, "sell deal succesfully ", current_number, file=f)
                break
    time.sleep(60)  # 每次轮询间隔60秒

with open(output_file, 'a') as f:
    print(stock_name, " buy and sell deal finished succefully !!!!!!!!!!!!!!!!!!!!! ", file=f)
